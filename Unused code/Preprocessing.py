...
def ColorConverter(directory, variant, total_images, output_dir, crop_mask, extention='.jpg', output_extention='.tif'):
    ...
    # gray_mask = cv.cvtColor(crop_mask, cv.COLOR_BGR2GRAY)
    ...
    for i in range(1, total_images + 1):
        ...
        if image is not None:
            ...
            # cropping using mask
            # cropped_rgb = cv.bitwise_and(blurred_image, crop_mask)
            # cropped_hsv = cv.bitwise_and(hsv, crop_mask)
            # cropped_lab = cv.bitwise_and(lab, crop_mask)
            
            transparent_rgb = getTransparentImage(blurred_image, crop_mask)
            transparent_hsv = getTransparentImage(hsv, crop_mask)
            transparent_lab = getTransparentImage(lab, crop_mask)

            # saving images into respective directories
            cv.imwrite(os.path.join(RGB_path, f"{variant}_RGB_{image_id}{output_extention}"), transparent_rgb)
            cv.imwrite(os.path.join(HSV_path, f"{variant}_HSV_{image_id}{output_extention}"), transparent_hsv)
            cv.imwrite(os.path.join(LAB_path, f"{variant}_LAB_{image_id}{output_extention}"), transparent_lab)
...
def getTransparentImage(src, mask):
    transparent_image = cv.cvtColor(src, cv.COLOR_BGR2BGRA)
...
def main():
    ...
    # memory_finish = tracemalloc.stop()
